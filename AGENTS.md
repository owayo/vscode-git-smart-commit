...
- Cancellation はキャンセルハンドラ内で `resolveOnce()` を呼ばず、`close` イベントでプロセスの終了を確認してから resolve する。さらに POSIX では `close` は「直接の子と stdio の終了」しか保証せず、stdio を継承しない孫プロセスが SIGTERM を無視して生き残るケースがあるため、`kill(-pid, 0)` の 50ms ポーリング (`waitForPosixProcessGroupExit`) でプロセスグループ全体の消滅を確認してから resolve する。ポーリング中は settled にならないため 5 秒後の SIGKILL 昇格タイマーが機能して残存プロセスを強制終了できる。SIGKILL でも消えない異常系 (uninterruptible sleep 等) は上限 10 秒で警告を出して resolve し、進捗表示が永久に残るのを防ぐ (ESRCH のみ消滅扱い、EPERM と非 `Error` throw は存続扱い)。キャンセル後に kill 失敗等で `error` が発火した経路も `close` と同じ消滅確認へ合流させる (spawn 失敗時は pid 未定義のため即 resolve に合流)。`error` と `close` が連続しても消滅待ちポーリングは 1 回だけ開始する。ポーリング開始時は「残存グループを即 SIGKILL する」shutdown ハンドラを active map へ再登録し、ポーリング中に VS Code の reload / deactivate が来ても残存グループを取りこぼさない (`close`/`error` 冒頭の map 削除だけだと deactivate 時に何も送れず孫が孤児化する)。さらにユーザーキャンセル後かつ `close` / `error` 前に deactivate が重なる区間でも、元の active map ハンドラが POSIX は SIGKILL、Windows は `taskkill /T /F` の再実行へ昇格する。これにより、キャンセル直後に終了未確定のまま VS Code 上で成功扱いになったり、拡張ホスト終了で昇格タイマーを失って `git-sc` の子孫プロセスが裏で走り続けたりする事象を防ぐ。
- 実行中の `git-sc` 子プロセスは `spawnGitScProcess.ts` の module-level `Map<ChildProcess, () => void>` で「プロセス → 終了ハンドラ」を追跡し (spawn 後に登録、`close`/`error` で delete)、拡張機能の `deactivate()` から `terminateActiveGitScProcesses(outputChannel)` を呼んで一括終了する。これにより VS Code の reload / 終了 / 拡張停止時に、POSIX で `detached` 起動した `git-sc` とその子孫が孤児として残らないようにする。拡張ホスト終了後は SIGKILL 昇格タイマーを実行できないため、shutdown ハンドラは `isCancelled` を立てた上で POSIX のプロセスグループへ即 SIGKILL を送り、Windows では `taskkill /T /F` でプロセスツリーを強制終了する。これにより通常の deactivate とユーザーキャンセル直後の deactivate の双方で SIGTERM 無視プロセスを取り残さず、拡張停止に伴う `close` も「キャンセル」扱いにして誤った成功/失敗通知や `git.refresh` を出さない。
...
- **Maintenance (Biome patch更新とsecurity override更新)**: `depup --install --include-pinned` で `@biomejs/biome` の宣言範囲を ^2.5.4 から ^2.5.5 へ更新した（lockfileの解決版と `biome.jsonc` のschemaは引き続き2.5.6）。最新の `pnpm audit --audit-level moderate` で修正版が要求された `brace-expansion` 5.0.9、`fast-uri` 3.1.5、`js-yaml` 4.3.1、`nanoid` 3.3.17、`postcss` 8.5.23、`undici` 7.29.0 を `pnpm-workspace.yaml` のoverrideへ反映し、`dependencyOverrides.test.ts` の有効行完全一致による退行テストも同時に更新した。監査は `No known vulnerabilities found` へ回復し、Lint / Format / TypeScript compile / 311テスト / VSIX packageはすべて成功。v8 coverageはStatements 99.14% / Functions 100%。全体レビューでは確実な追加バグや必要なリファクタリングは見つからず、`astro-sight review --dir . --git` も未解決影響 / API変更 / dead symbolなし。
- **Maintenance (Biome 更新と schema 整合テスト)**: `depup --install --include-pinned` で `@biomejs/biome` の宣言範囲を ^2.5.3 から ^2.5.4 へ更新し、lockfile では 2.5.6 を解決した。`biome migrate --write` で `biome.jsonc` の `$schema` も 2.5.6 へ揃え、今後の依存更新時に migrate 漏れを検出できるよう、インストール済み Biome package の実バージョンと有効な schema URL のバージョンが完全一致する回帰テストを `dependencyOverrides.test.ts` に追加した。`pnpm audit --audit-level moderate` は clean、Lint / Format / TypeScript compile / 311 テスト / VSIX package はすべて成功。全体レビューでは確実な追加バグや必要なリファクタリングは見つからず、`astro-sight review --dir . --git` も unresolved impact / missing cochange / API diff / dead symbols なし。
- **Reliability fix (通常の deactivate で SIGTERM 無視プロセスが孤児化)**: 実行中の `git-sc` が SIGTERM を無視する状態で VS Code の reload / 終了 / 拡張停止が起きると、従来の shutdown ハンドラは POSIX プロセスグループへ SIGTERM を 1 回送るだけだった。ユーザーキャンセル経路と異なり 5 秒後の SIGKILL 昇格タイマーを作らず、拡張ホストも終了するため、プロセスグループが孤児として残る確実な不具合を実プロセスで再現した (`{"processGroupStillAliveAfterSigterm":true}`)。通常の shutdown も `forceShutdownTerminate` へ統一し、POSIX は即 SIGKILL、Windows は従来どおり `taskkill /T /F` でプロセスツリーを強制終了するよう修正した。ユーザーキャンセル単体は引き続き SIGTERM → 5 秒後 SIGKILL の graceful な経路を維持する。pid 未定義時の `child.kill("SIGKILL")` フォールバックと、pid 定義時の負 PID への SIGKILL を回帰テストで固定し、総テスト 310 件、Statements 99.14% / Functions 100%。CodeRabbit の uncommitted review は findings 0、`astro-sight review --dir . --git` も unresolved impact / missing cochange / API diff / dead symbols なし。
- **Reliability / security maintenance (キャンセルと deactivate の pre-close race + audit 更新)**: CodeRabbit のクロスチェックで、ユーザーキャンセルが `isCancelled=true` にした後、`close` / `error` 前に deactivate が重なると active map の旧 shutdown ハンドラが早期 return し、POSIX の 5 秒後 SIGKILL タイマーが拡張ホスト終了とともに失われて SIGTERM 無視プロセスを孤児化させる確実な race を検出した。shutdown 時だけ POSIX は即 SIGKILL、Windows は `taskkill /T /F` を再実行する `forceShutdownTerminate` へ昇格するよう修正した。また `kill(-pid, 0)` の catch が `null` 等の非 `Error` throw に対して `.code` を参照して再 throw し、消滅待ちを停止する経路を、`Error` かつ ESRCH と安全に確認できた場合だけ消滅扱いにするよう修正。error + close の連続発火時にポーリングを二重開始しないこと、Windows キャンセル後の error が失敗通知を出さず resolve することも含め回帰テストを追加し、総テスト 309 件、Statements 99.15% / Functions 100%。あわせて `pnpm audit` で新規検出された `brace-expansion <=5.0.7` と `postcss <=8.5.17` を override で 5.0.8 / 8.5.18 へ更新し audit clean を回復した。CodeRabbit 再レビューは findings 0、`astro-sight review --dir . --git` も unresolved impact / missing cochange / API diff / dead symbols なし。
- **Reliability fix (POSIX キャンセルで SIGTERM 無視の孫プロセスが孤児化)**: codex (gpt-5.6-sol) のバグ探索クロスチェックで発見し、実プロセスで再現確認 (`{"directChildClosed":true,"processGroupStillAlive":true}`)。POSIX の `close` は「直接の子と stdio の終了」しか保証しないため、stdio を継承しない孫プロセスが SIGTERM を無視して残っていても、direct child の `close` で `resolveOnce()` が `forceKillTimer` をクリアし、5 秒後の SIGKILL 昇格が走らないまま孫が孤児として残っていた。キャンセル中の POSIX `close` では `kill(-pid, 0)` の 50ms ポーリング (`waitForPosixProcessGroupExit`) でプロセスグループ消滅を確認してから resolve するよう修正 (ポーリング中は settled にならず SIGKILL 昇格タイマーが機能する)。SIGKILL でも消えない異常系は上限 10 秒で警告を出して resolve し進捗表示の永久残留を防ぐ。ESRCH のみ消滅扱い、EPERM は存続扱い。`resolveOnce`/`rejectOnce` は `groupExitPollTimer` もクリアする。回帰テスト: グループ残存時の resolve 遅延 + SIGKILL 昇格後の resolve (commit / reword 両フロー)、上限超過時の警告付き resolve を追加し、既存の SIGKILL 昇格テスト 2 件は killSpy を「signal 0 に ESRCH を投げる (=グループ消滅済み)」実装へ更新して意図を保持した。
- **Test fix (security override 退行テストのコメント誤マッチ)**: `dependencyOverrides.test.ts` の `toContain` が YAML 全文を対象にしていたため、`# "js-yaml@...": "..."` のようにコメントアウトで無効化された override にも一致し、セキュリティ退行を見逃す余地があった (codex クロスチェックで指摘)。コメント行 (`#` 始まり) を除外した有効行だけを照合対象にするよう修正した。
- 前セッションの未コミット依存更新を検証して確定: typescript 6.0.3 → **7.0.2 (メジャー、ネイティブコンパイラ)**、vitest 4.1.10、`@vitest/coverage-v8` 4.1.10 を新規追加、Biome ^2.5.3 / @types/node ^26.1.1 / @vscode/vsce ^3.9.2 の範囲更新、`pnpm-workspace.yaml` の security overrides 更新 (brace-expansion 5.0.7 / fast-uri 3.1.4 / js-yaml 4.3.0 / linkify-it 5.0.2 追加等)。TS7 で `tsc -p ./` 型チェック・emit、`vsce package --no-dependencies`、`biome migrate` (no-op)、`pnpm audit --audit-level moderate` (clean) をすべて確認し、codex も tsconfig (commonjs/ES2022/declaration/sourceMap) との互換性に問題なしと独立確認した。`depup --install --include-pinned` は追加更新なし。
- `coverage/` (vitest --coverage の出力ディレクトリ) を `.gitignore` と `.vscodeignore` の双方に追加し、untracked のまま残っていた残骸を削除した。`tsconfig.tsbuildinfo` と同様、版管理・VSIX 配布の対象外とすべきビルド成果物のため。
- 非 Error 値が throw / reject された場合の `String(error)` 防御分岐 (到達可能だが未カバーだった) に回帰テスト 8 件を追加: `getGitWorkspaceRoot` の非 Error throw (commit / reword 両フロー)、`resolveSpawnCommand` の非 Error throw (通知 + Error への包み直し reject)、`git.refresh` の非 Error reject、インストール案内 `openExternal` の非 Error reject、履歴ロードの非 Error throw、SIGTERM 送信失敗の非 Error throw、`taskkill` 同期起動エラーの非 Error throw。退行すると通知が `[object Object]` になるため固定した。Branch カバレッジ 92.39% → 95%+、総テスト 304 件。CodeRabbit CLI (`coderabbit review --agent --type uncommitted`) は findings 0、`astro-sight review --dir . --git` も unresolved impact / missing cochange / API diff / dead symbols なし。
- `depup --install --include-pinned` は更新なしで、lockfile 上の Biome 2.5.3 / `@types/node` 26.1.1 が最新であることを確認した。`biome migrate --write` も no-op。あわせて重要だが未カバーだった 3 つの境界を回帰テストで固定した。(1) `deactivate()` の activate 前安全性テストは、従来 module cache を共有して先行テストの `activate()` 済み状態を誤って検証していたため、`vi.resetModules()` 後の新規モジュールで本当に `outputChannel` 未初期化の経路を通し、`terminateActiveGitScProcesses` が呼ばれないことも確認する。(2) Windows で `PATH` / `Path` / `path` がすべて未定義の場合、実行ファイル探索を一切行わず `null` を返す安全側の挙動を確認する。(3) 実行中の `git-sc` が 0 件の `terminateActiveGitScProcesses` は shutdown 警告を出さず静かに終了することを確認する。総テストは 293 件。コードベース全体を正確性・セキュリティ・エラー処理・性能の観点で再レビューし、astro-sight の全体構造・参照・dead-code 分析では未解決影響 / 未参照 export なし、CodeRabbit CLI の uncommitted review も findings 0 で、確実な実装バグや追加リファクタリングが必要な箇所は見つからなかった。ソースとテストのコメントも `astro-sight ast` で全件抽出し、日本語化方針を満たすことを確認した。
...
- Added regression tests for v8 カバレッジ計測により判明した「到達可能だが未カバーだった 5 つの実挙動」。(1) `resolveExecutableOnPath` の Windows で拡張子なし名が `PATHEXT` の全拡張子付き候補 (`git-sc.EXE` 等) に一致せず、拡張子なしの bare 名の実体へフォールバックする分岐 (`resolveExecutablePath.ts:170`)、(2) `resolveNativeExecutableOnPath` が POSIX では `resolveExecutableOnPath` へ委譲する分岐 (同 `:185`)、(3) `resolveWindowsSystemExecutable` が非 Windows では解決を試みず即 `null` を返す分岐 (同 `:227`)、(4) ユーザーキャンセル直後 (`isCancelled=true` かつ `close` 未達で active map に残存) に deactivate 相当の `terminateActiveGitScProcesses` が重なる race、(5) 正常 `close(0)` で `settled` になった後に遅延 `error` イベントが発火しても、error ハンドラが `settled` ガードで早期 return し、成功通知の後に失敗通知/`Failed to start git-sc` ログを二重に出さない race (当時の `spawnGitScProcess.ts:292`)。当初 (4) は shutdown ハンドラが早期 return して二重終了しないことを固定していたが、後続レビューで拡張ホスト終了時に SIGKILL 昇格タイマーを失い孤児化させるバグと判明したため、現在は shutdown 時に強制終了へ昇格するテストへ置換済み (本節先頭の maintenance note を参照)。当時の追加により対象 2 ファイル (`resolveExecutablePath.ts` / `spawnGitScProcess.ts`) の statement カバレッジを 100% へ、総テストを 289 件へ引き上げた。
