...
- **キャンセル時の安定動作** — 実行キャンセル時に不要な失敗通知を表示しない
- **履歴解析の堅牢化** — `Reword Commit` で `|`、`\x1f`、`\x1e` を含む件名も NUL 区切り解析で正しく扱える。`format:` プレフィックスにより複数コミット間のハッシュ汚染も防止
- **履歴取得失敗時の明示エラー** — `git log` 失敗時に「コミットなし」と誤表示せず原因を通知
- **未導入検知の強化** — Windows の `is not recognized` を含むエラーでもインストール案内を表示
