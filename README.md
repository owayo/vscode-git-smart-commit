...
- **実行時環境変数を反映** — Git ルート検出時にその時点の `PATH` など最新の環境変数を使い、古い環境状態を保持しない
- **キャンセル時の安定動作** — 実行キャンセル時に不要な失敗通知を表示しない。POSIX では `git-sc` プロセスを直接起動して `SIGTERM` を送り、Windows では `taskkill /T /F` でプロセスツリー全体を終了させるため、キャンセル後にバックグラウンドで処理が続く心配がない。さらに Windows で `taskkill` 自体の起動に失敗した場合も、拡張機能を落とさず出力チャンネルへ警告を記録する
...
- **Windows 実行ファイル解決の安全化** — `git-sc` を bare command で起動すると Windows の `CreateProcess` がカレントディレクトリを探索パスに含めるため、悪意あるリポジトリ直下に `git-sc.cmd` が置かれていれば優先実行されてしまう。この拡張は `PATH` を走査して絶対パス要素のみから実行ファイルを解決し、相対 PATH 要素 (`bin`、`.\tools` 等) を完全に除外。`.cmd`/`.bat` の場合のみ Node.js CVE-2024-27980 対策で `shell: true` を使用する。同様に `git` CLI 呼び出しも `cwd:` ではなく `git -C <dir>` 引数で作業ディレクトリを渡し、cwd ハイジャックを防ぐ
...
ユニットテストには、`extension.ts` で登録される各コマンドハンドラのオプション引き渡し、内部コマンド失敗時の例外抑止、キャンセル時の POSIX `SIGTERM` と Windows `taskkill` 分岐に関する回帰ケースも含まれます。
