...
dev 依存の推移依存には `pnpm-workspace.yaml` の `overrides` と lockfile 更新でパッチ済みバージョンを明示し、`pnpm audit --audit-level moderate` が通る状態を維持します。現在は `@isaacs/brace-expansion`、`ajv`、`brace-expansion`、`esbuild`、`fast-uri`、`form-data`、`js-yaml`、`linkify-it`、`lodash`、`markdown-it`、`minimatch`、`picomatch`、`postcss`、`qs`、`rollup`、`tmp`、`underscore`、`undici`、`vite` を patched version へ固定しています。`brace-expansion` は DoS 修正版の `5.0.8`、`postcss` は source map path traversal 修正版の `8.5.18`、`tmp` は path traversal 修正版の `0.2.7`、`undici` は監査で要求される `7.28.0` 以上へ上げています。これらの override は `dependencyOverrides.test.ts` で退行固定し、削除・バージョン後退で監査 clean 状態が崩れないようにしています。pnpm 11 の install script 承認は `allowBuilds` で `@vscode/vsce-sign`、`esbuild`、`keytar` を明示し、`pnpm install --frozen-lockfile` が非対話環境でも完了するようにします。Biome を更新するときは `pnpm exec biome migrate --write` も実行し、インストール済み Biome の実バージョンと `biome.jsonc` の schema URL が一致することを回帰テストで保証します。VS Code API 型定義を更新するときは `engines.vscode` も同じ最小バージョンへ揃え、`vsce package --no-dependencies` の整合チェックを通る状態を保ちます。
