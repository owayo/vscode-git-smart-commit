...
キャンセル状態機械については、ユーザーキャンセル後かつ `close` / `error` 前の deactivate で POSIX を SIGKILL へ昇格すること、プロセスグループ確認が非 `Error` 値を投げても待機を継続すること、`error` と `close` の連続発火で消滅待ちを二重開始しないこと、Windows のキャンセル後に `error` が発火しても失敗通知せず解決することも回帰テストで固定しています。全 309 テストに加え、v8 coverage は Statements 99.15% / Functions 100% を確認しています。
